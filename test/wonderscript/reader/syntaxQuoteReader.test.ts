...
describe("syntaxQuoteReader", () => {
    ...
    test("unquoting", () => {
        ...
        expect(form).toEqual([ARRAY_SYM, 1, Symbol.intern('x'), Symbol.intern('y')]);
    });
    test("unquoting and spliced unquoting together", () => {
        const { form: actual } = readString("`(1 ~x ~@ys)")[0];

        const expected = [SEND_SYM, [ARRAY_SYM, 1, Symbol.intern('x')], [Symbol.intern('concat'), Symbol.intern('ys'), [ARRAY_SYM]]];
        p(actual);
        p(expected);
    ...
    test("unquoting and spliced unquoting together", () => {
        ...
        const expected = [SEND_SYM, [ARRAY_SYM, [QUOTE_SYM, Symbol.intern('a')], Symbol.intern('x'), 4], [Symbol.intern('concat'), Symbol.intern('ys'), [ARRAY_SYM, 5]]];
        p(actual);
        p(expected);
        expect(actual).toEqual(expected);
    });
    ...
    test("spliced unquoting with 4 forms", () => {
        ...
        const expected = [SEND_SYM, [ARRAY_SYM, 1], [Symbol.intern('concat'), Symbol.intern('x'), [ARRAY_SYM, 3], [ARRAY_SYM, 4]]];
        p(actual);
        p(expected);
        expect(actual).toEqual(expected);
    });
